...
import {
  getBadHobbiesMessages,
  postBadHobbiesMessages,
  deleteBadHobbiesMessages,
} from "../../../../redux/actions/messagesActions";
import FlipMove from "react-flip-move";

const BadHobbies = forwardRef(
  (
    {
      generalMessages,
      auth,
    ...
    const onSubmitForm = (e) => {
      ...
      postBadHobbiesMessages(values);
      setInput("");
    };

    const deleteButtonMessage = () => {
      deleteBadHobbiesMessages(_id);
      setAnchorEl(null);
    };
      ...
      <>
        <div
          className={`ChatDashboard__ChatSystem ${
            generalMessages.loadingBad && "active"
          }`}
        >
          <FlipMove>
            {generalMessages.loadingBad ? (
              <CircularProgress size={100} style={{ color: "#7733FF" }} />
            ) : generalMessages?.badMessages?.length === 0 ? (
              <p
                style={{
                  color: "#7733FF",
                  textAlign: "center",
              ...
              generalMessages?.badMessages?.map(
